...
class CandidatoMenuTest {
    ...
    @Test
    void option3_callsUpdate() {
        inputService.setBr(new BufferedReader(new StringReader("3\n")));
        menuView.showCandidatoMenu();
        assertEquals(1, controller.updateCalls);
    }
    ...
    private static class RecordingMenuController extends MenuController {
        ...
        @Override
        public void handleCandidatoOption(int option) {
            switch (option) {
                case 1: createCalls++; break;
                case 2: readCalls++; break;
                case 3: updateCalls++; break;
